    ...
    try {
      ...
      recorder.onstop = async () => {
        try {
          ...
          const text = await sendAudioForSTT(audioBlob);
          ...
          console.log("Web STT error:", e);
          setSttText("STT error.");
        } finally {
          isUploadingRef.current = false;

          if (shouldContinueSpeechRef.current && activeTab === "speech") {
            setTimeout(() => {
              startWebRecording();
            }, 300);
          }
      ...
      console.log("startWebRecording error:", e);
      setSttText("Microphone permission denied or unavailable.");
    }
  };
    ...
    try {
      ...
      rec.setOnRecordingStatusUpdate((status) => {
        ...
        if (
          ...
          speechDurationRef.current >= 0.3 &&
          silenceTimerRef.current >= 1.0
        ) {
          if (isStoppingRef.current || isUploadingRef.current) return;
          stopRecordingAndSend(true, true);
        }
      ...
      console.log("Native startRecording error:", e);
      setIsRecording(false);
    }
  };
